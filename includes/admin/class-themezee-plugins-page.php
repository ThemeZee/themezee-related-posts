...
if ( ! class_exists( 'ThemeZee_Plugins_Page' ) ) :
	...
	class ThemeZee_Plugins_Page {
		...
		static function add_plugins_page() {
			...
			add_options_page(
				esc_html__( 'ThemeZee Plugins', 'themezee-related-posts' ),
				esc_html__( 'ThemeZee Plugins', 'themezee-related-posts' ),
				'manage_options',
				'themezee-plugins',
		...
		static function display_overview_page() {
			...
			<div id="themezee-plugins-overview">
				...
				<h1 id="themezee-plugin-header"><?php esc_html_e( 'ThemeZee Plugins', 'themezee-related-posts' ); ?></h1>

				<div class="themezee-plugins-intro">
					<?php printf( esc_html__( 'You need more features and functionality? Extend your website with our premium %s.', 'themezee-related-posts' ), $plugin_link ); ?>
				</div>
		...
		static function enqueue_admin_styles( $hook ) {
			...
			// Enqueue Admin CSS.
			wp_enqueue_style( 'themezee-plugins-stylesheet', TZRP_PLUGIN_URL . 'assets/css/themezee-plugins.css', array(), TZRP_VERSION );
		}
	}
