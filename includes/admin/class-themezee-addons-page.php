...
class ThemeZee_Addons_Page {
	...
	*/
	static function add_addons_page() {
		...
		add_theme_page(
			esc_html__( 'ThemeZee Add-ons', 'themezee-related-posts' ),
			esc_html__( 'Theme Add-ons', 'themezee-related-posts' ),
			'manage_options',
			'themezee-addons',
	...
	*/
	static function display_overview_page() { 
		...
		$addon_link = '<a target="_blank" href="http://themezee.com/addons/" title="'. esc_html__( 'ThemeZee Add-ons', 'themezee-related-posts' ) . '">'. esc_html__( 'add-ons', 'themezee-related-posts' ) . '</a>';
		?>
		
		<div id="themezee-addons-overview">
		
			<h1 id="themezee-addon-header"><?php esc_html_e( 'ThemeZee Add-ons', 'themezee-related-posts' ); ?></h1>
			<div class="themezee-addons-intro">
				<?php printf( esc_html__( 'You need more features and functionality? Extend your website with our customized %s.', 'themezee-related-posts' ), $addon_link ); ?>
			</div>
			<hr/>
	...
	static function get_settings_tabs() {
		...
		$tabs                 = array();
		$tabs['overview']      = esc_html__( 'Overview', 'themezee-related-posts' );
	...
	*/
	static function enqueue_admin_styles( $hook ) {
		...
		// Enqueue Admin CSS
		wp_enqueue_style( 'themezee-addons-stylesheet', TZRP_PLUGIN_URL . 'assets/css/themezee-addons.css', array(), TZRP_VERSION );
